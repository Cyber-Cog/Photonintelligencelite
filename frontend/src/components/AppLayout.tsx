...
/** Fixed top navbar height — keep in sync with `--app-nav-height` in CSS. */
export const APP_NAV_HEIGHT_CLASS = "h-16";
...
/** Full-viewport-width top bar — not a card; theme chrome + subtle border. */
export function TopNavbar() {
  ...
  return (
    <header className={`app-header app-header-fixed ${isLanding ? "app-header-landing" : ""}`}>
      <div
        className={`mx-auto flex ${APP_NAV_HEIGHT_CLASS} w-full max-w-none items-center gap-3 px-4 sm:gap-4 sm:px-5 lg:px-6`}
      >
 ...
 *
 * Edge shell (Results / job tools): full-width fixed navbar + flush canvas under it.
 * Hierarchy is Navbar → Sidebar | Main — never a rounded card wrapping both.
 */
export function AppLayout({ children }: { children: ReactNode }) {
  const { connecting, connectAttempt, apiUnreachable, refresh } = useAuth();
  const location = useLocation();
  const isLanding = location.pathname === "/";
  ...
  /** Full-bleed canvas under navbar — Results + job tool panes (not workflow forms). */
  const isEdgeShell = isResults || isLockedJobView;
  const shellMax = isEdgeShell || isProcessing || isWorkflow ? "max-w-none" : "max-w-6xl";
  ...
  useEffect(() => {
    document.documentElement.classList.toggle("app-shell-edge", isEdgeShell);
    return () => document.documentElement.classList.remove("app-shell-edge");
  }, [isEdgeShell]);
  ...
  if (isLanding) {
    mainClass = "relative flex w-full flex-1 flex-col";
  } else if (isResults) {
    mainClass = "app-shell-main relative flex w-full min-h-0 flex-1 flex-col";
  } else if (isLockedJobView) {
    mainClass =
      "app-shell-main app-shell-main-locked relative flex w-full min-h-0 flex-1 flex-col overflow-hidden";
  } else if (isProcessing) {
    mainClass = `relative mx-auto flex w-full ${shellMax} min-h-0 flex-1 flex-col overflow-hidden py-2 ${shellPadX}`;
  } else {
    mainClass = `relative mx-auto flex w-full ${shellMax} flex-1 flex-col py-5 ${shellPadX}`;
  }

  return (
    <div
      className={`app-layout relative flex flex-col bg-[color:var(--pic-surface)] text-[color:var(--pic-text)] ${
        fillViewport ? "h-dvh overflow-hidden" : "min-h-screen"
      }`}
    >
      {!isLanding && !isEdgeShell ? <div className="hero-mesh" aria-hidden /> : null}
      {connecting ? (
        <div
      ...
      <MainContent className={mainClass}>{children}</MainContent>

      {!isEdgeShell ? (
        <footer
          className={`shrink-0 app-footer ${isLanding ? "app-footer-landing" : ""} ${
            isProcessing ? "app-footer-compact" : ""
          }`}
        >
          <div
            className={`mx-auto flex w-full ${shellMax} flex-wrap items-center justify-center gap-x-1.5 gap-y-1 ${shellPadX}`}
          >
