"""Wide single-header: Timestamp + INV1_P / plant ICR Inverter N metrics → long tidy."""
...
def try_wide_single_header(matrix: list[list[str]], *, sheet_name: str) -> StrategyResult | None:
    ...
    for i, row in enumerate(rows[:30]):
        ...
        wide_hits = sum(1 for c in row if c.strip() and parse_wide_device_column(c.strip()) is not None)
        s += wide_hits * 0.4
        if s > best:
            best = s
            header_idx = i
    ...
    used = {m for _, m, _ in col_map.values()}
    can_derive = (
    ...
    if icr_ids and "ICR ID" not in out_header:
        if "Equipment ID" in out_header:
            ei = out_header.index("Equipment ID")
            out_header.insert(ei + 1, "ICR ID")
        else:
            out_header.insert(1, "ICR ID")
    ...
    for row in rows[header_idx + 1 :]:
        ...
        for j, (dev, metric, _icr) in col_map.items():
